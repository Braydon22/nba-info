...
function SingleTeam(props) {
    ...
    const fetchData = async () => {
        console.log(`${gameUrl}?team_id[]=${teamId}&start_date=${startDateStr}&end_date=${endDateStr}`)
        setLoading(true)
        try {
            const teamUrlResponse = await fetch(`${teamUrl}/${teamId}`)
            const teamData = await teamUrlResponse.json()
            const gameUrlResponse = await fetch(`${gameUrl}?team_ids[]=${teamId}&start_date=${startDateStr}&end_date=${endDateStr}`)
    ...
    return (
        <section className='single-team-view'>
            ...
            <div className="game-info-container" style={{ marginTop: '20vh' }}>
                <h2> Recent / Upcoming Games for {team.full_name}</h2>
